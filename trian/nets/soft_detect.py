...
class SoftDetect(LightningModule):
    ...
    def detect_keypoints(self, scores_map, normalized_coordinates=True):
        ...
        for b_idx in range(b):
            ...
            # modified
            a = torch.linspace(-self.radius, self.radius, self.kernel_size).to(scores_map.device)
            mask_x = torch.exp(-(a[None, :] - xy_residual[:, 0][:, None]) * (a[None, :] - xy_residual[:, 0][:, None]))
            mask_x = torch.stack((mask_x, mask_x, mask_x, mask_x, mask_x), 1).view(-1, 25)  # Mx25
            mask_y = torch.exp(-(a[None, :] - xy_residual[:, 1][:, None]) * (a[None, :] - xy_residual[:, 1][:, None]))
            mask_y = torch.stack((mask_y, mask_y, mask_y, mask_y, mask_y), 2).view(-1, 25)

            a = (self.hw_grid[None, :, :] - xy_residual[:, None, :])  # Mx25x2
            mask_xy = torch.exp(-torch.abs(a.sum(dim=2) * 0.707))  # Mx25
            mask_yx = torch.exp(-torch.abs((a[:, :, 0] - a[:, :, 1]) * 0.707))  # Mx25
            ...
            scoredispersity = torch.stack(
                [scoredispersity_x, scoredispersity_y, scoredispersity_xy, scoredispersity_yx], 1)  # Mx4
            scoredispersity1, i = scoredispersity.max(dim=1)
            mask_i = torch.ones_like(scoredispersity, dtype=torch.bool, device=scoredispersity.device)
            mask_i.scatter_(1, i.unsqueeze(1), False)
            scoredispersity = torch.gather(scoredispersity, 1, mask_i.nonzero()[:, 1].view(-1, 3))
            scoredispersity2, i = scoredispersity.max(dim=1)
            scoredispersity = 0.5 * (scoredispersity1 + scoredispersity2)
